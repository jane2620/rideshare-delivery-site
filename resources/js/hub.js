...
document.getElementById('submitZipButton').onclick = async (event) => {
    ...
    try {
        ...
        if (uploadTag === "dataonly") {
            window.location.href = "https://princetonsurvey.az1.qualtrics.com/jfe/form/SV_8HbhWLqxBT6I0Qu";
            return;
        }
        // ETA capture flow
        if (uploadTag === "uber_eta_capture") {
            window.location.href = "https://princetonsurvey.az1.qualtrics.com/jfe/form/SV_abBB0VYZjyvDuiq";
            return;
        }
        // Delivery upload flow: route to delivery-specific Qualtrics after successful upload
        if (uploadTag === "delivery") {
            window.location.href = "https://princetonsurvey.az1.qualtrics.com/jfe/form/SV_6GdeWEOSM2N84B0";
            return;
        }
        
        // Enable screenshot submit button now that ride data is uploaded
        updateScreenshotSubmitButtonState();
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (email) {
        ...
        if (screenshotEmailInput) screenshotEmailInput.value = email;
        else if (screenshotPidInput) screenshotPidInput.value = email;
        ...
        if (continueButton && returnInput) {
            continueButton.addEventListener('click', () => {
                ...
                if (enteredEmail) {
                    ...
                    if (screenshotEmailInput) screenshotEmailInput.value = enteredEmail;
                    else if (screenshotPidInput) screenshotPidInput.value = enteredEmail;
...
function initializeScreenshotUpload() {
    ...
    // Don't reset if already initialized (check if listeners exist)
    ...
    // Update button state when email changes
    const screenshotEmailInput = document.getElementById('screenshotEmailInput');
    const screenshotPidInput = document.getElementById('screenshotProlificIdInput'); // Backward compatibility
    const screenshotInput = screenshotEmailInput || screenshotPidInput;
    if (screenshotInput) {
        screenshotInput.addEventListener('input', updateScreenshotSubmitButtonState);
    }
}

function getScreenshotEmail() {
    const emailInput = document.getElementById('screenshotEmailInput');
    if (emailInput) return emailInput.value.trim();
    // Backward compatibility
    const pidInput = document.getElementById('screenshotProlificIdInput');
    return pidInput ? pidInput.value.trim() : "";
}

function getScreenshotProlificId() {
    return getScreenshotEmail();
}
...
function updateScreenshotSubmitButtonState() {
    const submitBtn = document.getElementById('submitScreenshotsButton');
    if (submitBtn) {
        // Button is disabled if: no email, no files, OR ride data not uploaded yet
        submitBtn.disabled = !(getScreenshotProlificId() && filesToUpload.length > 0 && rideDataUploaded);
    }
}
...
async function submitScreenshots() {
    ...
    try {
        ...
        const studyType = document.body ? document.body.dataset.studyType : "";
        const screenshotPayload = { pid: pid, fileName: fileName };
        if (studyType) {
            screenshotPayload.studyType = studyType;
        }
        ...
        if (!signedUrlResponse.ok) {
            ...
            throw new Error(`Failed to get upload URL: ${signedUrlResponse.status} ${signedUrlResponse.statusText} - ${errorText}`);
        }
        ...
        successDiv.innerHTML = `
            ...
            <a href="${qualtricsLink}" class="button is-success is-medium" target="_blank" style="margin-top: 1rem; display: block;">
